...
import { useTransactions, useDataLoaded } from '@/store/useDashboardStore'
...
export default function MonthlyComparison() {
  ...
  const availableMonths = useMemo(() => {
    const months = new Set(transactions.filter((t) => t.type === 'expense').map((t) => t.date.slice(0, 7)))
    return Array.from(months).sort().reverse()
  }, [transactions])

  const [month1, setMonth1] = useState<string>(availableMonths[0] || '')
  const [month2, setMonth2] = useState<string>(availableMonths[1] || '')

  const month1Transactions = useMemo(() => getMonthTransactions(transactions, month1), [transactions, month1])
  const month2Transactions = useMemo(() => getMonthTransactions(transactions, month2), [transactions, month2])

  const month1Total = useMemo(() => month1Transactions.reduce((s, t) => s + t.amount, 0), [month1Transactions])
  const month2Total = useMemo(() => month2Transactions.reduce((s, t) => s + t.amount, 0), [month2Transactions])
  const totalChange = month2Total - month1Total
  const totalChangePercent = month1Total > 0 ? (totalChange / month1Total) * 100 : 0

  const month1Categories = useMemo(() => aggregateByCategory(month1Transactions, 'expense'), [month1Transactions])
  const month2Categories = useMemo(() => aggregateByCategory(month2Transactions, 'expense'), [month2Transactions])
  ...
  const month1Merchants = useMemo(() => aggregateByMerchant(month1Transactions), [month1Transactions])
  const month2Merchants = useMemo(() => aggregateByMerchant(month2Transactions), [month2Transactions])
  ...
  const month1Daily = useMemo(() => aggregateByDay(month1Transactions, 'expense'), [month1Transactions])
  const month2Daily = useMemo(() => aggregateByDay(month2Transactions, 'expense'), [month2Transactions])
        ...
        {!dataLoaded ? (
          <div className="flex min-h-[calc(100vh-120px)] items-center justify-center">
            <div className="w-full max-w-md text-center">
              ...
              <h2 className="text-xl font-semibold text-slate-200">暂无账单数据</h2>
              <p className="mt-2 text-sm text-slate-500">请先导入消费账单后再使用月度对比功能</p>
              <Link
                to="/"
                className="mt-6 inline-flex items-center gap-2 rounded-lg bg-emerald-500/15 px-4 py-2 text-sm text-emerald-400 transition-colors hover:bg-emerald-500/25"
              >
          ...
          <div className="flex min-h-[calc(100vh-120px)] items-center justify-center">
            <div className="w-full max-w-md text-center">
              ...
              <h2 className="text-xl font-semibold text-slate-200">月份数据不足</h2>
              <p className="mt-2 text-sm text-slate-500">
                当前仅有 {availableMonths.length} 个月的数据，至少需要 2 个月才能进行对比分析
              </p>
